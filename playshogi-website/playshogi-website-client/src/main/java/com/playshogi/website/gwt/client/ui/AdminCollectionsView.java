...
@Singleton
public class AdminCollectionsView extends Composite {

    interface MyEventBinder extends EventBinder<AdminCollectionsView> {
    }
    ...
    @Inject
    public AdminCollectionsView(final PlaceController placeController, final SessionInformation sessionInformation,
                                final AppPlaceHistoryMapper historyMapper) {
        GWT.log("Creating admin collections view");
        this.sessionInformation = sessionInformation;
        problemsTable = new ProblemCollectionsTable(historyMapper, true, false);
        gamesTable = new GameCollectionsTable(historyMapper, true, false);

        HtmlContentBuilder<HTMLDivElement> div = Elements.div();
        div.css(Styles.padding_20);

        div.add(Elements.h(1).textContent("Admin Collections"));
    ...
    public void activate(final EventBus eventBus) {
        GWT.log("Activating admin collections view");
    ...
    public void onListGameCollectionsEvent(final ListGameCollectionsEvent event) {
        GWT.log("AdminCollectionsView: handle GameCollectionsEvent:");
    ...
    public void onListProblemCollections(final ListProblemCollectionsEvent event) {
        GWT.log("AdminCollectionsView: handle ListProblemCollectionsEvent");
        if (event.getPublicCollections() != null) {
